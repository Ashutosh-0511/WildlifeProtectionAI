...
class QwenUnavailableError(RuntimeError):
    """Raised when the Qwen fallback cannot serve the video request."""


def _endpoint() -> str:
    base = os.getenv("QWEN_BASE_URL", DEFAULT_BASE_URL).rstrip("/")
...
def _sample_frames(video_path: Path, max_frames: int = MAX_FRAMES) -> list[tuple[float, str]]:
    ...
    if not capture.isOpened():
        raise QwenUnavailableError("Unable to open video for fallback analysis")

    try:
        frame_count = int(capture.get(cv2.CAP_PROP_FRAME_COUNT))
        fps = float(capture.get(cv2.CAP_PROP_FPS) or 0.0)
        if frame_count <= 0:
            raise QwenUnavailableError("Video contains no readable frames")
        ...
        if not sampled:
            raise QwenUnavailableError("Unable to sample readable frames from video")
        return sampled
    finally:
        capture.release()
...
def _extract_text(response: requests.Response) -> str:
    ...
    if not choices:
        raise QwenUnavailableError("Qwen returned no choices")
...
def _parse_json(text: str) -> dict[str, Any]:
    ...
    try:
        ...
        if start < 0 or end <= start:
            raise QwenUnavailableError("Qwen returned non-JSON output")
        try:
            parsed = json.loads(cleaned[start : end + 1])
        except json.JSONDecodeError as exc:
            raise QwenUnavailableError("Qwen returned invalid JSON") from exc
    if not isinstance(parsed, dict):
        raise QwenUnavailableError("Qwen response was not a JSON object")
    return parsed
...
def _write_result(
    ...
    data["status"] = "success"
    data["model"] = os.getenv("QWEN_MODEL", DEFAULT_MODEL)
...
def analyze_video(
    ...
    *,
    ...
    model_name = os.getenv("QWEN_MODEL", DEFAULT_MODEL).strip() or DEFAULT_MODEL
    max_frames = max(1, int(os.getenv("QWEN_MAX_FRAMES", str(MAX_FRAMES))))
    ...
    try:
        ...
        raise QwenUnavailableError("Qwen fallback was unavailable") from exc

    if not data:
        raise QwenUnavailableError("Qwen returned an empty analysis")
